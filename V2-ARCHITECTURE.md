...
## Momentec groundwork (2.0.3)

Momentec Brands is registered as supplier key `momentec` in the multi-supplier layer. Settings exist for staging/production, API base URL, account/customer number, username/client ID, and API key/secret. Live endpoints and authentication are intentionally not guessed before credentials are issued. The future adapter will normalize Momentec catalog and inventory into the same exact sparse variation/source model used by SanMar and S&S.
