...
class ASSS_MultiSupplier {
    ...
    public static function suppliers(): array {
        return ['sanmar'=>'SanMar', 'ss'=>'S&S Activewear'];
    }
    ...
    public function settings(): array {
        ...
        $priority = array_values(array_filter(array_map('sanitize_key', preg_split('/\s*,\s*/', (string)($s['supplier_priority'] ?? 'ss,sanmar')) ?: [])));
        $priority = array_values(array_unique(array_filter($priority, static fn($v) => in_array($v, ['ss','sanmar'], true))));
        foreach (['ss','sanmar'] as $supplier) if (!in_array($supplier, $priority, true)) $priority[] = $supplier;
    ...
    public function save_product_sources(int $product_id, array $sources): void {
        ...
        foreach ($sources as $supplier=>$data) {
            ...
            if (!in_array($supplier, ['sanmar','ss'], true) || !is_array($data)) continue;
    ...
    public function register_product_source(int $product_id, string $supplier, array $data): void {
        ...
        if (!in_array($supplier, ['sanmar','ss'], true)) return;
    ...
    public function remove_product_source(int $product_id, string $supplier, string $reason = ''): bool {
        $supplier = sanitize_key($supplier);
        if (!in_array($supplier, ['sanmar','ss'], true)) return false;
        $sources = $this->product_sources($product_id);
    ...
    public function recalculate_variation_inventory(int $variation_id): array {
        ...
        if($parent_id>0){
            ...
            if(in_array($preferred,['ss','sanmar'],true)){
    ...
    public function product_coverage(int $product_id): array {
        $coverage = ['sanmar'=>0,'ss'=>0,'both'=>0,'total'=>0];
        foreach ($this->variation_ids($product_id) as $variation_id) {
    ...
    public function source_counts(): array {
        $counts = ['sanmar_products'=>0,'ss_products'=>0,'multi_products'=>0,'total_products'=>0];
