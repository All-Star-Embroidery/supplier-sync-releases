...
class ASSS_Admin {
    ...
    public function __construct($s, $ss, $i, $y, $m, $u) {
        $this->sanmar = $s;
        $this->ss = $ss;
        $this->importer = $i;
        $this->sync = $y;
    ...
    public function actions(): void {
        ...
        if (!empty($_POST['asss_save_product_supplier_settings'])) {
            ...
            if(!in_array($preferred,['','ss','sanmar'],true))$preferred='';
        ...
        if (!empty($_POST['asss_disconnect_product_supplier'])) {
            ...
            if(!$product_id || !in_array($supplier,['sanmar','ss'],true)) $this->redir('asss-intelligence','','Invalid product or supplier.');
    ...
    public function add_products(): void {
        ...
        if (!in_array($supplier, ['sanmar','ss'], true)) $supplier = 'sanmar';
        $this->wrap_start($supplier === 'ss' ? 'Add S&S Activewear Products' : 'Add SanMar Products');
        $this->notice();
    ...
    public function active_products_page(): void {
        ...
        foreach ($ids as $id) {
            ...
            if ($supplier === 'multi' || count(array_filter($sources, static fn($v)=>is_array($v)&&!empty($v['enabled']))) > 1) {
    ...
    private function is_active_linked_product(int $product_id): bool {
        ...
        if (!in_array((string)get_post_meta($product_id, '_asss_supplier', true), ['sanmar','ss','multi'], true)) return false;
    ...
    public function manage_suppliers_page(): void {
        ...
        foreach(['sanmar','ss'] as $src){$d=$sources[$src]??null;if(!$d)continue;$label=ASSS_MultiSupplier::suppliers()[$src];echo '<div class="card" style="padding:16px"><h3 style="margin-top:0">'.esc_html($label).'</h3>';
        ...
        foreach($vids as $vid){$v=wc_get_product((int)$vid);if(!$v instanceof WC_Product_Variation)continue;$intel=$this->multi->variation_intelligence((int)$vid);if(!$intel)continue;$attrs=$v->get_variation_attributes();$labels=[];foreach($attrs as $tax=>$slug){$tax=str_replace('attribute_','',$tax);$term=taxonomy_exists($tax)?get_term_by('slug',$slug,$tax):null;$labels[]=($term&&!is_wp_error($term))?$term->name:$slug;}
            ...
            foreach(['sanmar','ss'] as $src){$r=$intel[$src]??null;if(!$r){echo '<td>—</td>';continue;}echo '<td><strong>'.($r['quantity']===null?'—':number_format($r['quantity'])).'</strong> stock<br><small>Cost '.($r['cost']===null?'—':'$'.number_format($r['cost'],2)).'<br>SKU '.esc_html($r['sku']?:'—').($r['gtin']?'<br>GTIN '.esc_html($r['gtin']):'').'</small></td>';}
    ...
    public function brands_page(): void {
        $supplier = sanitize_key((string)($_GET['supplier'] ?? 'sanmar'));
        if (!in_array($supplier, ['sanmar','ss'], true)) $supplier = 'sanmar';
        $title = $supplier === 'ss' ? 'S&S Activewear Brands' : 'SanMar Brands';
    ...
    public function settings(): void {
        ...
        echo $this->row('Supplier priority', 'supplier_priority', $s['supplier_priority'] ?? 'ss,sanmar');
        echo '<tr><th></th><td><p class="description">Comma-separated supplier keys. Recommended: <code>ss,sanmar</code>. Priority chooses the suggested fulfillment source, the source used by Preferred-only inventory mode, and the wholesale basis for automatic pricing when that supplier carries the exact variation.</p></td></tr>';
