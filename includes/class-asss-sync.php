...
class ASSS_Sync {
    ...
    public function repair_product_job(int $product_id): void {
        ...
        if(!$product_id || !in_array((string)get_post_meta($product_id,'_asss_supplier',true), ['sanmar','ss','momentec','multi'], true) || (string)get_post_meta($product_id,'_asss_sync_enabled',true)!=='yes') return;
    ...
    public function queue_product_repairs(array $product_ids): array {
        ...
        foreach($ids as $index=>$product_id) {
            if(!in_array((string)get_post_meta($product_id,'_asss_supplier',true), ['sanmar','ss','momentec','multi'], true) || (string)get_post_meta($product_id,'_asss_sync_enabled',true)!=='yes') { $skipped++; continue; }
            $already=false;
    ...
    public function linked_product_ids(bool $include_archived = true, string $supplier = ''): array {
        ...
        foreach ($q->posts as $id) {
            ...
            if (!in_array($linked, ['sanmar','ss','momentec','multi'], true)) continue;
            if ($supplier !== '') {
