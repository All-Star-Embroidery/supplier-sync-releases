...
class ASSS_Importer {
    ...
    private function canonical_supplier_size(string $brand, string $style, string $size): string {
        ...
        if (!is_array($maps) || empty($maps[$product_key]) || !is_array($maps[$product_key])) return '';
        return (string)($maps[$product_key][$size_key] ?? '');
    }
...
", $parts));
    ...
    private function normalize_ss_media_url(string $url): string {
        ...
        return esc_url_raw('https://www.ssactivewear.com/' . ltrim($url, '/'));
    }
    ...
    private function is_supplier_attachment(int $attachment_id): bool {
        ...
        return in_array((string)get_post_meta($attachment_id, '_asss_image_source', true), ['sanmar','ss','supplier'], true);
    }
    ...
    private function sync_ss_parent_media(int $product_id, array $data, array $variants): void {
        ...
        if (!$product) return;
        $urls = [];
        foreach ([(string)($data['images']['product'] ?? ''), (string)($data['images']['thumbnail'] ?? '')] as $raw) {
        ...
        if (!$current || $this->is_supplier_attachment($current)) $product->set_image_id($ids[0]);
        $manual_gallery = [];
    ...
    public function link_ss_style_to_product(int $product_id, int $brand_id, int $style_id, array $selected_colors = []) {
        ...
        ASSS_Logger::log('S&S linked as additional supplier',$missin['product_id'=>$product_id,'matched_existing'=>$matched,'created_ss_only'=>$created]);
        return $product_id;
    }
    ...
    public function link_sanmar_style_to_product(int $product_id, string $brand, string $style, array $selected_colors = []) {
        ...
        if (!$selected_colors) $selected_colors=$all_colors;
        $lookup=array_fill_keys($selected_colors,true);
        $rows=array_values(array_filter($rows,function($row)use($lookup){$c=(string)$this->sanmar->first($rown['COLOR_NAME','COLOR','CATALOG_COLOR']);return $c!==''&&isset($lookup[$c]);}));
        $rows=$this->dedupe_supplier_rows($this->hydrate_color_images($rows));
        ...
        $expected=[]; $matched=0; $created=0;
        foreach($rows as $row){
            $color=trim((string)$this->sanmar->first($rown['COLOR_NAME','COLOR','CATALOG_COLOR']));
            $size=trim((string)$this->sanmar->first($rown['SIZE']));
            $unique=trim((string)$this->sanmar->first($rown['UNIQUE_KEY','UNIQUEKEY','PART_ID']));
            ...
            if(!$vid){$r=$this->create_or_update_variation($product_id,$brand,$style,$row);if(is_wp_error($r))continue;$vid=(int)$r['variation_id'];$created++;}
                ...
                update_post_meta($vid,'_asss_sanmar_inventory_key',(string)$this->sanmar->first($rown['INVENTORY_KEY','INVENTORYKEY']));
                update_post_meta($vid,'_asss_sanmar_size_index',(string)$this->sanmar->first($rown['SIZE_INDEX','SIZEINDEX']));
                ...
                update_post_meta($vid,'_asss_sanmar_cost',(string)$this->sanmar->first($rown['PIECE_PRICE','PRICE']));
                update_post_meta($vid,'_asss_sanmar_case_price',(string)$this->sanmar->first($rown['CASE_PRICE']));
                update_post_meta($vid,'_asss_sanmar_sale_price',(string)$this->sanmar->first($rown['PIECE_SALE_PRICE']));
                update_post_meta($vid,'_asss_sanmar_map_price',(string)$this->sanmar->first($rown['MAP_PRICE']));
                $existing_v=wc_get_product($vid);$weight=(string)$this->sanmar->first($rown['PIECE_WEIGHT']);
            ...
            $this->multi->register_variation_source($vid,'sanmar',[
                'brand'=>$brand,'style'=>$style,'unique_key'=>$unique,
                'inventory_key'=>(string)$this->sanmar->first($rown['INVENTORY_KEY','INVENTORYKEY']),
                'size_index'=>(string)$this->sanmar->first($rown['SIZE_INDEX','SIZEINDEX']),
                'color'=>$color,'size'=>$size,'weight_lb'=>(string)$this->sanmar->first($rown['PIECE_WEIGHT']),
                'cost'=>(string)$this->sanmar->first($rown['PIECE_PRICE','PRICE']),
                'case_price'=>(string)$this->sanmar->first($rown['CASE_PRICE']),
                'sale_price'=>(string)$this->sanmar->first($rown['PIECE_SALE_PRICE']),
                'map_price'=>(string)$this->sanmar->first($rown['MAP_PRICE']),
                'product_status'=>(string)$this->sanmar->first($rown['PRODUCT_STATUS','STATUS']),
        ...
        ASSS_Logger::log('SanMar linked as additional supplier',$missin['product_id'=>$product_id,'matched_existing'=>$matched,'created_sanmar_only'=>$created]);
        return $product_id;
    }
    ...
    private function sanmar_gallery_urls(array $row): array {
        ...
        foreach(['COLOR_PRODUCT_IMAGE','FRONT_FLAT','THREE_Q_MODEL','SIDE_MODEL','BACK_MODEL','BACK_FLAT','FRONT_MODEL'] as $field){
            $url=trim((string)$this->sanmar->first($rown[$field]));
