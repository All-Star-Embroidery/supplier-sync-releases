...
class ASSS_Importer {
    ...
    public function __construct(ASSS_SanMar $sanmar, ASSS_SS $ss, ASSS_Momentec $momentec, ASSS_MultiSupplier $multi) {
        ...
        add_action('admin_init', [$this, 'migrate_momentec_shipping_weights_v2022'], 39);
        add_action('admin_init', [$this, 'migrate_discovery_taxonomy_and_titles_v2023'], 40);
    ...
    private function is_one_size_value(string $size): bool {
    ...
    private function create_or_update_variation(int $product_id, string $brand, string $style, array $row, int $variation_id = 0) {
        ...
        $v->update_meta_data('_asss_supplier_cost', (string)$this->sanmar->first($row, ['UNIT_BUY_PRICE','PIECE_PRICE','PRICE']));
        $v->update_meta_data('_asss_sanmar_cost', (string)$this->sanmar->first($row, ['UNIT_BUY_PRICE','PIECE_PRICE','PRICE']));
...
", $parts));
    ...
    /** Apply supplier shipping weight without overwriting a merchant-owned edit. */
    ...
    private function category_signal_text(int $product_id, array $raw_categories): string {
    ...
    private function prune_orphaned_supplier_categories(array $term_ids): void {
    ...
    /** v2.0.26: standardize existing supplier cost records and rebuild managed pricing. */
    ...
    /** v2.0.24: queue existing SanMar-owned featured images once after upgrade. */
    ...
    private function sync_parent_media(int $product_id, array $row): void {
        ...
        // v2.0.24: queue the chosen SanMar storefront source for GitHub-side
    ...
    /** v2.0.28: repair supplier-generic storefront titles without overwriting merchant edits. */
    ...
    private function is_supplier_attachment(int $attachment_id): bool {
        ...
        return in_array((string)get_post_meta($attachment_id, '_asss_image_source', true), ['sanmar','sanmar-normalized','ss','momentec','supplier'], true);
    }
    ...
    public function link_sanmar_style_to_product(int $product_id, string $brand, string $style, array $selected_colors = []) {
        ...
        foreach($rows as $row){
            ...
            if(!$vid){$r=$this->create_or_update_variation($product_id,$brand,$style,$row);if(is_wp_error($r))continue;$vid=(int)$r['variation_id'];$created++;}
                ...
                update_post_meta($vid,'_asss_sanmar_cost',(string)$this->sanmar->first($row,['UNIT_BUY_PRICE','PIECE_PRICE','PRICE']));
    ...
    private function sync_momentec_bulk_order_fields(WC_Product_Variable $product,array $data,array $variants,bool $is_new): void {
        ...
        foreach($variants as $row){$size=trim((string)($row['size'] ?? ''));if($size!==''){$size=$this->storefront_size_label($brand,$style,$size);$sizes[$size]=true;}}
        if(!empty($settings['sync_bulk_order_fields'])){
            if((string)$product->get_meta('_asbo_display_name')===''&&$title!=='')$product->update_meta_data('_asbo_display_name',$title);
            if((string)$product->get_meta('_asbo_short_description')===''&&$description!=='')$product->update_meta_data('_asbo_short_description',wp_trim_words($description,55,'…'));
            if((string)$product->get_meta('_asbo_size_chart')===''){
    ...
    private function reconcile_momentec_variations(int $product_id,array $rows,bool $allow_create=true): array {
        ...
        foreach($seen as $combo=>$vid){$v=wc_get_product($vid);if(!$v instanceof WC_Product_Variation)continue;$row=$expected[$combo];if($v->get_regular_price('edit')===''||$v->get_price('edit')==='')$missing_price++;if($v->get_sku('edit')==='')$missing_sku++;$expected_weight=$row['weight_lb'] ?? null;if($expected_weight!==null&&is_numeric($expected_weight)&&(float)$expected_weight>0&&$v->get_weight('edit')==='')$missing_weight++;if(!empty($this->sanmar->settings()['sync_images'])&&!empty($row['gallery'])&&(string)$v->get_meta('_asss_momentec_media_pending')!=='yes'){$saved=$v->get_meta('_asss_variation_gallery_ids');if(is_string($saved)){$d=json_decode($saved,true);if(is_array($d))$saved=$d;}if(!$v->get_image_id())$missing_image++;if(count((array)$row['gallery'])>1&&count((array)$saved)<2)$missing_gallery++;}}
        $audit=['supplier'=>'momentec','expected'=>count($expected),'supplier_variations'=>count($seen),'missing_expected'=>max(0,count($expected)-count($seen)),'missing_price'=>$missing_price,'missing_image_when_available'=>$missing_image,'missing_variation_gallery'=>$missing_gallery,'missing_sku'=>$missing_sku,'missing_weight'=>$missing_weight,'created'=>$created,'updated'=>$updated,'checked_at'=>current_time('mysql')];
        update_post_meta($product_id,'_asss_last_variation_audit',wp_json_encode($audit));
        ASSS_Logger::log(array_sum([$audit['missing_expected'],$missing_price,$missing_image,$missing_gallery,$missing_sku,$missing_weight])?'Momentec variation audit completed with issues':'Momentec variation audit passed',array_sum([$audit['missing_expected'],$missing_price,$missing_image,$missing_gallery,$missing_sku,$missing_weight])?'warning':'info',['product_id'=>$product_id,'audit'=>$audit]);
        return $audit;
    }
