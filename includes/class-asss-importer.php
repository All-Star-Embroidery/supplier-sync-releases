...
class ASSS_Importer {
    ...
    private function pricing_supplier_order(int $product_id): array {
        ...
        if (in_array($preferred, ['ss','sanmar'], true)) $order[] = $preferred;
        foreach ((array)($settings['priority'] ?? ['ss','sanmar']) as $supplier) {
            $supplier = sanitize_key((string)$supplier);
            if (in_array($supplier, ['ss','sanmar'], true) && !in_array($supplier, $order, true)) $order[] = $supplier;
        }
        foreach (['ss','sanmar'] as $supplier) if (!in_array($supplier, $order, true)) $order[] = $supplier;
        return $order;
    }
    ...
    private function variation_pricing_basis(int $product_id, int $variation_id): ?array {
        ...
        if ($legacy_cost !== '' && is_numeric($legacy_cost) && (float)$legacy_cost > 0) {
            ...
            if (!in_array($supplier, ['ss','sanmar'], true)) $supplier = sanitize_key((string)get_post_meta($product_id, '_asss_supplier', true));
            if (!in_array($supplier, ['ss','sanmar'], true)) $supplier = $this->pricing_supplier_order($product_id)[0] ?? 'sanmar';
